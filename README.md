...
# It sets a working dir in "/app" , copies files from package*.json . Install all required depdencecies with npm ci (npm ci is more faster and reliable then npm install). The next step is copying the application and exposes port 3000. For running a healtcheck I was using node because it already exists in the image. If i wanted to use curl, I would have to install it which increases space usage. 
